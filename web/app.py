...
class _RecognitionServiceFallback:

    """Safe fallback so the web server can still run without crashing."""
    ...
    def set_yolo_active(self, _enabled: bool) -> bool:
    ...
    def recognize_from_base64(self, _image_data):
...
try:
    ...
    logger.exception("Failed to initialize recognition service")

    logger.warning("Server will continue in degraded mode until recognition is available")

    recognizer = _RecognitionServiceFallback()
...
def _recognize_or_error(image_data):
    ...
    if not recognizer.load_encodings():
